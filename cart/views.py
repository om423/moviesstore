...
# NEW: handle survey POST (optional)

def create_feedback(request):
	if request.method == 'POST':
		name = (request.POST.get('name') or "").strip()
		comment = (request.POST.get('comment') or "").strip()
		# Survey is optional; only save if they typed a comment
		if comment:
			CheckoutFeedback.objects.create(name=name, comment=comment)
		# After submit (or cancel), go to the feedback list so the TA can verify
		return redirect('cart.feedback_list')
	return redirect('home.index')

# NEW: public page listing all short statements
